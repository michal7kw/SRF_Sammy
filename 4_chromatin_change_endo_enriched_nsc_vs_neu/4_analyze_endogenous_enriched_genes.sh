#!/bin/bash
#SBATCH --job-name=4_analyze_endogenous_enriched_genes
#SBATCH --output=logs/4_analyze_endogenous_enriched_genes_%a.out
#SBATCH --error=logs/4_analyze_endogenous_enriched_genes_%a.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
# Start logging
log "Starting endogenous Mecp2 enriched gene analysis for array task ${SLURM_ARRAY_TASK_ID}"
...
if [ ! -f "4_analyze_endogenous_enriched_genes.py" ]; then
    log "ERROR: 4_analyze_endogenous_enriched_genes.py not found in $WORKDIR"
    exit 1
fi

# Check if gene list file exists
if [ ! -f "gene_lists/endo_enriched_gene_list_nsc_vs_neu.txt" ]; then
    log "ERROR: Gene list file not found in gene_lists directory"
    exit 1
fi
...
python 4_analyze_endogenous_enriched_genes.py --chrom "${CHROM}" --gtf_file "$GTF_FILE"
