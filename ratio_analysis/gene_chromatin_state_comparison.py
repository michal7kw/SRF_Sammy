...
def load_chromatin_states(results_dir, condition, state_type):
    ...
    possible_paths = [
        os.path.join(results_dir, "chromatin_states", f"{condition}_{state_type}.bed"),
        os.path.join(results_dir, "py_implementation", "chromatin_states", f"{condition}_{state_type}.bed")
    ]
    ...
    if state_file is None:
        logger.error(f"Chromatin state file not found for {condition}_{state_type}. Tried paths: {possible_paths}")
        return pd.DataFrame()
    ...
    try:
        ...
        logger.info(f"Loaded {len(states)} {state_type} regions for {condition} from {state_file}")
        return states
    except Exception as e:
        logger.error(f"Error loading chromatin states from {state_file}: {e}")
        return pd.DataFrame()
