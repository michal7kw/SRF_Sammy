...
This script analyzes chromatin state changes between NSC_GFP and Neu_GFP conditions over larger genomic windows,
...
def analyze_broad_chromatin_changes(results_dir, window_size, genome_size, ratio_change_threshold=0.2, 
                                   ...
                                   debug=False):
    """
    Analyze chromatin state changes between NSC_GFP and Neu_GFP conditions over broad genomic regions
    """
    bedgraph_dir = os.path.join(results_dir, "bedgraph")
    # Create output directory for shifted regions
    shifted_regions_dir = os.path.join(results_dir, "shifted_regions_NSC_NEU")
    os.makedirs(shifted_regions_dir, exist_ok=True)
    
    if debug:
        logger.info(f"Bedgraph directory: {bedgraph_dir}")
        logger.info(f"Shifted regions directory: {shifted_regions_dir}")
    
    # Define comparisons
    comparisons = [
        {
            'name': 'NSC_GFP vs Neu_GFP',
            'nsc_ratio': os.path.join(bedgraph_dir, 'NSC_GFP_S2S_vs_S3_ratio.bedgraph'),
            'neu_ratio': os.path.join(bedgraph_dir, 'Neu_GFP_S2S_vs_S3_ratio.bedgraph'),
        }
    ]
    ...
    for comp in comparisons:
        ...
        if not os.path.exists(comp['nsc_ratio']) or not os.path.exists(comp['neu_ratio']):
        ...
        # Read ratio files
        try:
            nsc_df = pd.read_csv(comp['nsc_ratio'], sep='\t', header=None, 
                                 names=['chrom', 'start', 'end', 'score'])
            neu_df = pd.read_csv(comp['neu_ratio'], sep='\t', header=None, 
                               names=['chrom', 'start', 'end', 'score'])
            
            if debug:
                logger.info(f"  NSC ratio file: {len(nsc_df)} regions")
                logger.info(f"  Neu ratio file: {len(neu_df)} regions")
                logger.info(f"  NSC ratio range: {nsc_df['score'].min()} to {nsc_df['score'].max()}")
                logger.info(f"  Neu ratio range: {neu_df['score'].min()} to {neu_df['score'].max()}")
        except Exception as e:
            logger.error(f"Error reading ratio files for {comp['name']}: {e}")
            continue
        
        # Aggregate data into larger windows
        logger.info(f"  Aggregating NSC data into {window_size}bp windows...")
        nsc_windows = aggregate_into_windows(nsc_df, window_size)
        
        logger.info(f"  Aggregating Neu data into {window_size}bp windows...")
        neu_windows = aggregate_into_windows(neu_df, window_size)
        
        if debug:
            logger.info(f"  NSC windows: {len(nsc_windows)}")
            logger.info(f"  Neu windows: {len(neu_windows)}")
        ...
        merged_windows = merged_windows[
            (merged_windows['coverage_nsc'] >= min_coverage) & 
            (merged_windows['coverage_neu'] >= min_coverage)
        ]
        
        if debug:
            logger.info(f"  Windows with good coverage: {len(merged_windows)}")
        
        # Calculate the change in log2 ratio (Neu - NSC)
        merged_windows['ratio_change'] = merged_windows['score_neu'] - merged_windows['score_nsc']
        ...
        if debug:
            ...
            # Statistical test - are the NSC and Neu distributions significantly different?
            t_stat, p_value = ttest_ind(
                merged_windows['score_nsc'], 
                merged_windows['score_neu'],
                equal_var=False
            )
...
def plot_broad_chromatin_changes(results, window_results, output_file, window_size, ratio_change_threshold=0.2):
    ...
    # Set style parameters - FIX for compatibility with newer matplotlib versions
    try:
        ...
        try:
            ...
            # Fall back to default style with grid
    ...
    for i, comp_name in enumerate(comparisons):
        ...
        if comp_name in window_results:
            ...
            ax2.set_xlabel('Log2 Ratio Change (Neu - NSC)', labelpad=8)
            ax2.set_ylabel('Density', labelpad=8)
            ax2.set_title(f'{comp_name} - Ratio Changes', pad=10)
            ax2.grid(True, alpha=0.3)
    
    # 3. Scatter plots comparing NSC vs Neu values
    for i, comp_name in enumerate(comparisons):
        ...
        if comp_name in window_results:
            ...
            sc = ax3.scatter(data['score_nsc'], data['score_neu'], 
            ...
            min_val = min(data['score_nsc'].min(), data['score_neu'].min())
            max_val = max(data['score_nsc'].max(), data['score_neu'].max())
            ...
            # Add labels with improved formatting
            ax3.set_xlabel('NSC Log2 Ratio', labelpad=8)
            ax3.set_ylabel('Neu Log2 Ratio', labelpad=8)
    ...
    simple_output = os.path.splitext(output_file)[0] + '_simple.png'
    create_side_by_side_chart(results, simple_output)
    ...
    try:
        ...
        logger.error(f"Error saving data to {tsv_file}: {e}")
...
def create_side_by_side_chart(results, output_file):
    ...
    try:
        ...
        logger.error(f"Error saving simple chart to {output_file}: {e}")
...
def main():
    ...
    if args.debug:
        ...
        logger.debug(f"Results directory: {args.results_dir}")
